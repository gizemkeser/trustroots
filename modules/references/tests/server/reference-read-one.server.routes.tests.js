    ...
    it('read a single public reference by id', function (done) {
      agent
        ...
        .end(function (err, res) {
          ...
          try {
            const userFromExp = _.pick(users[1], userProfile.userMiniProfileFields.split(' ').slice(2));
            userFromExp._id = users[1]._id.toString();
            const userToExp = _.pick(users[2], userProfile.userMiniProfileFields.split(' ').slice(2));
